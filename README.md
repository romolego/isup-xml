...
Первым всегда выбирается основной XSD-файл (например, ATISO.xsd, AOOK.xsd, GeneralWorkJournal.xsd).

Далее добавляются все остальные XSD-файлы из подпапок — Types и другие папки, содержащие файлы .xsd (например, BuildingInfo, Participants, WorkInfo и т. д.).
...
под кнопками появятся счётчики:
Файлов, Глоб. элементов, Типов, Максимальная глубина, Строк всего;

Файлов, Глоб. элементов, Типов — статические (берутся из загруженных схем);

«Максимальная глубина» — показывает наибольший уровень вложенности элементов в схеме (например, если самый глубокий элемент вложен на 8 уровней, значение будет 8);
этот показатель определяется один раз при разборе XSD и не изменяется при фильтрации;

«Строк всего» — динамический параметр, который пересчитывается каждый раз при изменении фильтров, сворачивании или раскрытии дерева.
...
Цветовая индикация уровней — включает подсветку дерева по уровням вложенности (см. раздел ниже).
...
Фильтры динамические — любое изменение чекбоксов (в фильтрах или дереве) применяется мгновенно, без кнопки «Применить».
...
Кнопки копирования и добавления в фильтры
...
Уровень раскрытия — задаёт глубину видимых уровней дерева.
Можно выбрать:

конкретное число (например, до 10-го уровня включительно);

либо вариант «Все», чтобы полностью раскрыть дерево без ограничений.
...
Все панели (например, «Настройки отображения», «Фильтры отображения групп», «Фильтр разворачивания групп») можно сворачивать и разворачивать стрелкой слева от заголовка.
...
7. Сохранить результат через «Сохранить snapshot» или экспортировать дерево в TXT.XSD
