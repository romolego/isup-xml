...
2. Нажмите **«Загрузить и разобрать XSD»**.  
   После этого:
   - под кнопками появятся счётчики: **Файлов**, **Глоб. элементов**, **Типов**, **Максимальная глубина**, **Строк всего**;
   - **Файлов**, **Глоб. элементов**, **Типов** — статические (берутся из загруженных схем);
   - **Максимальная глубина** — показывает наибольший уровень вложенности элементов в схеме. Определяется один раз при разборе XSD и не изменяется при фильтрации;
   - **Строк всего** — динамический параметр, который пересчитывается каждый раз при изменении фильтров или сворачивании дерева.
...
## 6. Выбранная ветка и раскрытие

- **Отобразить только выбранную ветку** — оставляет в дереве только ветвь, выбранную в панели навигации.
...
- **Экспорт CSV** — выгружает текущее состояние дерева (с учётом фильтров и разворота) в CSV для дальнейшей обработки.
...
6. При необходимости включить **«Отобразить только выбранную ветку»**.
7. Сохранить результат через **«Сохранить snapshot»** или экспортировать дерево в **TXT**/**CSV**.

---

## Репозиторий

[romolego.github.io/isup-xml/](https://romolego.github.io/isup-xml/)

---

### Краткое описание

XSD Viewer — лёгкий клиент для разбора и визуализации XSD-схем.  
Рекомендуется для аналитиков и конфигураторов, работающих с XML-документами ИСУП и смежными системами.
